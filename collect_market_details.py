...
def flows(secid, end):
    if not secid.startswith('90.'):
        ...
        if not rows: raise ValueError('No dated stock flow rows')
        return [{'date':r['日期'][:10],'net_yuan':r['主力净流入-净额'],'net_ratio_pct':r['主力净流入-净占比']} for r in rows if r['日期'][:10]<=end][-35:]
...
def main():
    ...
    if len(benchmark)<22:
        raise ValueError('Dashboard must contain at least 22 verified benchmark trading dates')
    benchmark = sorted(benchmark, key=lambda r:r['date'])
    if len({r['date'] for r in benchmark}) != len(benchmark):
    ...
    if args.reuse:
        for key in list(jobs):
            if key in datasets and datasets[key].get('rows'):
                availability.append({'dataset':key,'status':'cached','rows':len(datasets[key]['rows']),'fetched_at':datasets[key]['fetched_at']})
                del jobs[key]
    if args.offline:
        availability.extend({'dataset':k,'status':'error','reason':'Not available in saved responses'} for k in jobs)
        jobs={}
    with ThreadPoolExecutor(max_workers=4) as ex:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                availability.append({'dataset':key,'status':'ok','rows':len(rows)})
                print(key,len(rows),flush=True)
            except Exception as exc:
                availability.append({'dataset':key,'status':'error','reason':type(exc).__name__+': '+str(exc)[:180], 'retained':key in datasets})
    ...
    for mapping in [SECTORS,stocks]:
        ...
        for code,name in mapping.items():
            ...
            if code in stocks and not any(r['date']==target for r in p):
                ...
                if snapshot:
                    p=p+[{'date':target,'close':snapshot.get('最新价'),'return_pct':snapshot.get('涨跌幅'),'turnover_yuan':snapshot.get('成交额'),'turnover_pct':snapshot.get('换手率')}]
    ...
    etf=[]; prevmap={r['基金代码']:r for r in rows('etf_'+prev)}
    for row in rows('etf_'+target):
        previous=prevmap.get(row['基金代码'])
        if previous and str(row['统计日期'])[:10]==target and str(previous['统计日期'])[:10]==prev:
            curr=number(row['基金份额']);before=number(previous['基金份额'])
            if curr is not None and before is not None and before>0:
                etf.append({'code':row['基金代码'],'name':row['基金简称'],'shares':curr,'previous_shares':before,'delta_shares':curr-before,'change_pct':(curr/before-1)*100})
    if not etf and old.get('etf_comparison_dates') == [prev,target]:
        etf=old.get('etf_changes',[])
    etf.sort(key=lambda r:abs(r['delta_shares']),reverse=True)
    result={'updated_at':now.isoformat(),'data_date':target,'benchmark':'上证指数000001；未复权收盘收益，超额为百分点差',
        'sector_scope':'东方财富板块代码：传媒、农林牧渔、石油石化、半导体；仅4个关注板块，含不同层级，不能加总为全市场，也不拼接新闻中的申万资金序列',
        'limit_scope':'东方财富涨停专题池；接口文档注明不含ST及科创板，未验证北交所完整覆盖。与新闻41只口径独立；封板资金为快照，首次至最后封板间隔不等于回封速度',
        'etf_scope':'上交所ETF；份额单位为份，差值为净份额变化，不是人民币净申购金额；仅比较同代码和明确日期',
    ...
    for dataset in result['datasets'].values():
        rows_=dataset['rows']
        if rows_ and 'date' in rows_[0]:
            ds=[r['date'] for r in rows_]
            if len(ds)!=len(set(ds)) or max(ds)>target:
                raise ValueError('Duplicate or future-dated history')
