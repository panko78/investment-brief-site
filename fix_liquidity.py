...
def retry_call(fn,label,attempts=3,base_sleep=.8):
    last=None
    for i in range(attempts):
        try:
            value=fn()
            if value is None: raise RuntimeError("empty response")
            return value
...
def fetch_all_a_margin(start_date,end_date):
    ...
    for d in sorted(sh_map)[-12:]:
        ...
        try:
            ...
            sz_bal=safe_float(sz.iloc[0].get('融资融券余额')); bj_bal=safe_float(bj.iloc[0].get('融资融券余额'))
            if sz_bal is None or bj_bal is None: continue
            result.append({'date':d,'margin_balance':round(sh_map[d]+sz_bal+bj_bal/10000.0,2)})
...
def fetch_hs_turnover_official():
    rows=[]
    for d in recent_weekdays(14):
        ds=d.strftime('%Y%m%d')
        try:
            sse=retry_call(lambda ds=ds: ak.stock_sse_deal_daily(date=ds),f"SSE turnover {ds}",attempts=2,base_sleep=.25)
            ...
            sz=retry_call(lambda ds=ds: ak.stock_szse_summary(date=ds),f"SZSE turnover {ds}",attempts=2,base_sleep=.25)
...
def eastmoney_index_amount(secid,start_date,end_date):
    ...
    def _fetch():
        r=requests.get(url,params=params,headers=UA,timeout=(8,20)); r.raise_for_status(); j=r.json()
        lines=(j.get('data') or {}).get('klines') or []
        if not lines: raise RuntimeError('no kline rows')
        return lines
    out={}
    for line in retry_call(_fetch,f"Eastmoney amount {secid}",attempts=4):
        a=line.split(',')
        if len(a)>=7:
            v=safe_float(a[6])
...
def fetch_sh_index(start_date,end_date):
    df=None
    try: df=retry_call(lambda: ak.stock_zh_index_daily_em(symbol='sh000001',start_date=start_date,end_date=end_date),'Shanghai Composite EM',attempts=3)
    except Exception as exc: print('Shanghai Composite EM failed, fallback Sina:',exc)
    if df is None or df.empty: df=retry_call(lambda: ak.stock_zh_index_daily(symbol='sh000001'),'Shanghai Composite Sina',attempts=3)
    if df is None or df.empty: raise RuntimeError('Shanghai Composite data is empty')
    dc=next((c for c in df.columns if str(c).lower()=='date' or str(c)=='日期'),None)
    cc=next((c for c in df.columns if str(c).lower()=='close' or str(c)=='收盘'),None)
    ...
    if not rows: raise RuntimeError('No Shanghai Composite observations')
    if rows[-1]['date']!=expected_market_date(): raise RuntimeError(f"Shanghai index latest {rows[-1]['date']} is stale")
    return rows
...
def main():
    ...
    today=datetime.now().date(); start=(today-timedelta(days=45)).strftime('%Y%m%d'); end=today.strftime('%Y%m%d')
    warnings=[]
    try:
        fresh_margin=fetch_all_a_margin(start,end)
        margin_series=merge_rows(old.get('margin_series') or old.get('series'),fresh_margin,'margin_balance')
    except Exception as exc:
        print('margin update failed, keep previous:',exc)
        margin_series=old.get('margin_series') or [{'date':x['date'],'margin_balance':x['margin_balance']} for x in old.get('series',[]) if x.get('margin_balance') is not None]
        warnings.append('两融接口本次失败，保留上一成功数据和真实数据日')
    try:
        fresh_turnover,turnover_source_short=fetch_hs_turnover(start,end)
        turnover_series=merge_rows(old.get('turnover_series') or old.get('series'),fresh_turnover,'turnover')
    except Exception as exc:
        print('turnover update failed, keep previous:',exc)
        turnover_series=old.get('turnover_series') or [{'date':x['date'],'turnover':x['turnover']} for x in old.get('series',[]) if x.get('turnover') is not None]
        turnover_source_short='上一成功数据'; warnings.append('成交额主源和备用源均失败，保留上一成功数据和真实数据日')
    try:
        fresh_index=fetch_sh_index(start,end); sh_index_series=merge_rows(old.get('sh_index_series'),fresh_index,'sh_close')
    except Exception as exc:
    ...
    legacy=[{'date':d,'margin_balance':mm[d],'turnover':tm[d]} for d in common]
    data['liquidity']={
        'status':'多源校验更新正常' if not warnings else '；'.join(warnings),
        'freshness':{'margin_as_of':margin_series[-1]['date'],'turnover_as_of':turnover_series[-1]['date'],'sh_index_as_of':sh_index_series[-1]['date'],'checked_at':datetime.now().strftime('%Y-%m-%d %H:%M')},
        'margin_series':margin_series[-30:],'turnover_series':turnover_series[-30:],'sh_index_series':sh_index_series[-30:],'series':legacy,
        'high_30d':{'margin_balance':max(margin_series,key=lambda x:x['margin_balance']),'turnover':max(turnover_series,key=lambda x:x['turnover']),'sh_index':max(sh_index_series,key=lambda x:x['sh_close'])},
        'margin_source':'上交所+深交所+北交所融资融券汇总（全A；增量更新）',
        'turnover_source':'优先上交所+深交所官方汇总；失败自动切东方财富上证A股指数000002+深证A股指数399107；当前：'+turnover_source_short,
        'sh_index_source':'上证指数000001：东方财富主源，新浪备用源',
        'source':'各序列独立记录真实数据日期；页面更新时间不代表所有序列同日披露'
    }
