...
def fetch_liquidity(data):
    # Pull a wider calendar window so Chinese holidays do not prevent us
    # from retaining a full 30 actual trading-day observations.
    days=last_weekdays(45)
    ...
    try:
        ...
        for d in days:
            ...
            try:
                ...
                sz_bal=safe_float(sz.iloc[0]["融资融券余额"])
        ...
        if out:
            ...
            data["liquidity"]={
                ...
                "series":series,
                "high_30d":{
                    "margin_balance":max(series,key=lambda x:x["margin_balance"]),
                    "turnover":max(series,key=lambda x:x["turnover"])
                },
                "source":"上交所融资融券汇总 + 深交所融资融券汇总；成交额为沪市A股+科创板及深市主板A股+创业板A股"
            }
    except Exception as e:
...
def comex_copper_hist_usd_ton():
    try:
        ...
        return pd.DataFrame({"date":pd.to_datetime(df["日期"]),"close":pd.to_numeric(df["最新价"],errors="coerce")*2204.62262185}).dropna()
    except Exception as e:
        ...
        try:
            ...
            if key=="copper":
                ...
                if df is None or df.empty:
                    ...
                    if df is not None and not df.empty: data_source="COMEX铜连续备用（HG00Y，已折算美元/吨）"
            ...
            if vals:
                ...
                elif key=="copper": item["source_note"]="当前使用LME 3个月铜历史行情；若该接口为空会自动切换COMEX铜连续备用源。"
            else:
                item=old.get(key,{**cfg,"latest":None,"month_change":None,"trend":"数据不足","outlook":"等待行情源更新","series":[]})
                item.update({"drivers":cfg["drivers"],"risks":cfg["risks"],"unit":cfg["unit"],"symbol":cfg["symbol"]})
            result[key]=item
        except Exception as e:
...
def fetch_tungsten_public(old_item):
    base=old_item or {
        "name":"钨","symbol":"中国APT（88.5% WO₃）现货/长协参考","unit":"元/吨","latest":600000,
        "month_change":None,"trend":"高位震荡","outlook":"等待更多公开报价更新",
        "drivers":["国内钨矿资源约束","出口管制与全球供应安全溢价","硬质合金、军工、航空航天和半导体需求"],
        "risks":["下游淡季需求偏弱","高价抑制补库","矿端供应恢复"],
        "series":[{"date":"2026-07-31","close":634000},{"date":"2026-08-05","close":606000},{"date":"2026-08-12","close":600000},{"date":"2026-08-14","close":605000},{"date":"2026-08-25","close":600000}]
    }
    base["source_note"]="SMM公开钨市场资讯；钨无LME式连续期货，本项采用中国APT现货/长协公开报价。"
    try:
        html=requests.get("https://news.metal.com/minor-metals/tungsten",headers=UA,timeout=20).text
        from bs4 import BeautifulSoup
        soup=BeautifulSoup(html,"html.parser")
        links=[]
        ...
        latest_price=None; latest_date=None
        for url in links[:15]:
            try:
                body=BeautifulSoup(requests.get(url,headers=UA,timeout=15).text,"html.parser").get_text(" ",strip=True)
                patterns=[r'APT[^.]{0,180}?(?:at|to|around|assessed at|quoted at)[^\d]{0,30}(?:RMB|Yn|yuan)?\s*([0-9]{3},[0-9]{3})\s*(?:yuan|RMB|/mt|/ton|per ton)',r'(?:RMB|Yn|yuan)\s*([0-9]{3},[0-9]{3})\s*(?:/mt|/ton|per ton)[^.]{0,120}?APT',r'APT[^.]{0,180}?\s([0-9]{3},[0-9]{3})\s*yuan/mt']
                vals=[]
                for pat in patterns:
                    for m in re.finditer(pat,body,re.I):
                        v=int(m.group(1).replace(",",""))
                        if 100000<=v<=2000000: vals.append(v)
                if vals:
                    latest_price=vals[-1]
                    dm=re.search(r'(?:Published:?\s*)?(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\s+(\d{1,2}),\s+(20\d{2})',body,re.I)
                    latest_date=datetime.strptime(f"{dm.group(1)} {dm.group(2)} {dm.group(3)}","%b %d %Y").date().isoformat() if dm else datetime.now().date().isoformat()
                    break
        ...
        if latest_price:
            ...
            found=False
            for row in series:
                if row.get("date")==day: row["close"]=latest_price; found=True; break
            if not found: series.append({"date":day,"close":latest_price})
            series=sorted(series,key=lambda x:x["date"])[-35:]
            vals=[float(x["close"]) for x in series]
            base["latest"]=latest_price; base["series"]=series
            if len(vals)>=2:
                pct=(vals[-1]/vals[0]-1)*100; base["month_change"]=round(pct,2)
    ...
    except Exception as e: print("tungsten public update failed:",e)
    return base
