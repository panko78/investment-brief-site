...
def live_projection(ranking,data_date=None):
    ...
    for row in rows[:MAX_SECTORS]:
        sectors.append({'code':None,'name':row.get('name'),'data_date':as_of,'return_pct':row.get('return_pct'),'turnover_yuan':None,'turnover_pct':None,'day_net_yuan':row.get('day_net_yuan'),'net_to_turnover_pct':row.get('day_net_ratio_pct'),'turnover_vs_prev20':None,'windows':{'3':{'net_yuan':row.get('three_day_net_yuan'),'inflow_days':None,'return_pct':row.get('three_day_return_pct'),'excess_pct_point':None,'observed_flow_days':3 if row.get('three_day_net_yuan') is not None else 0},'5':{'net_yuan':row.get('five_day_net_yuan'),'inflow_days':None,'return_pct':row.get('five_day_return_pct'),'excess_pct_point':None,'observed_flow_days':5 if row.get('five_day_net_yuan') is not None else 0},'10':{'net_yuan':row.get('ten_day_net_yuan'),'inflow_days':None,'return_pct':row.get('ten_day_return_pct'),'excess_pct_point':None,'observed_flow_days':10 if row.get('ten_day_net_yuan') is not None else 0}}})
    return sectors
...
def main():
    ...
    if now.weekday()<5 and (9,30)<=hm<(15,10):
        ...
        if len(sectors)<MIN_COMPLETE: raise ValueError(f'Live sector ranking incomplete: {len(sectors)} rows')
        market['sectors']=sectors; market['sector_mode']='live_ranking'; market['sector_scope']='动态行业资金榜：当日、3日、5日、10日资金均来自当前行业排名；盘中不伪造成交强度、流入天数或相对收益。'; market['sector_ranking_updated_at']=ranking.get('updated_at'); market['sector_ranking_source']=ranking.get('source'); MARKET.write_text(json.dumps(market,ensure_ascii=False,indent=2,allow_nan=False)+'\n',encoding='utf-8'); print('SAVED live dynamic sectors',len(sectors),flush=True); return
    ...
    if len(sectors)<MIN_COMPLETE: sectors=live_projection(ranking,target); market['sector_mode']='completed_ranking_fallback'; market['sector_scope']='动态行业资金榜；完整历史抓取不足，保留最近完成交易日的当日/3日/5日/10日资金。'
